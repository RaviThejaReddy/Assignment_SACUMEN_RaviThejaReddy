...
def do_the_process():
    ...
    [covet_log_to_dict_from_file(path_to_watch + "/" + f)
     for f in before.keys()]
    [os.replace(path_to_watch + "/" + f, path_for_processed_files + "/" + f +
                f".{datetime.datetime.now().strftime('%Y_%m_%dT%H_%M_%S_%fZ')}") for f in before.keys()]
    before = {}
    while 1:
        ...
        if added:
            [covet_log_to_dict_from_file(path_to_watch + "/" + f)
             for f in added]
            # [threading.Thread(target=covet_log_to_dict_from_file, args=(path_to_watch + "/" + f,)).start() for f in added]
            print
            print(f'added {added}')
            [os.replace(path_to_watch + "/" + f, path_for_processed_files + "/" + f +
                        f".{datetime.datetime.now().strftime('%Y_%m_%dT%H_%M_%S_%fZ')}") for f in added]
